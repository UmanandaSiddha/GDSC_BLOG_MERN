...
export const addEmailToQueue = async (data: EmailData) => {
    const result = await redis.xlen("bull:gdsc_email-queue:events");
    if (result > 0) {
        await redis.del("bull:gdsc_email-queue:events");
    }

    const id = await redis.get("bull:gdsc_email-queue:id");
    if (Number(id) > 99) {
        await redis.set("bull:gdsc_email-queue:id", "0");
    }
